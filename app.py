...
# Cache PDF content in memory
@lru_cache(maxsize=1)
def fetch_pdf_content():
    response = requests.get(PDF_URL)
    if response.status_code == 200:
        return BytesIO(response.content)
    raise Exception("Unable to fetch the PDF.")
...
# Function to process each page for highlighting
def process_page(page_num, page, page_line_pairs):
    pdf_content = fetch_pdf_content()
...
def extract_lines(text):
    ...
    """
    pattern = r'\(hrhbtb-page-(\d+)\.pdf, Lines (\d+)-(\d+)\)'
...
def view_pdf():
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            pages = list(executor.map(lambda p: process_page(p[0], p[1], page_line_pairs), enumerate(reader.pages, start=1)))
